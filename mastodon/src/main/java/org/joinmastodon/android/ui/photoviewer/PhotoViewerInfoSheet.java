...
public class PhotoViewerInfoSheet extends BottomSheet{
	...
	public void setStatus(Status status, String accountID){
		statusID=status.id;
		boolean isReblogPermitted=status.isReblogPermitted(accountID);
		boostBtn.setCompoundDrawablesWithIntrinsicBounds(0, switch(status.visibility){
			case DIRECT -> R.drawable.ic_fluent_arrow_repeat_all_24_regular;
			case PUBLIC, UNLISTED, LOCAL -> R.drawable.ic_boost;
			case PRIVATE -> isReblogPermitted ? R.drawable.ic_boost : R.drawable.ic_fluent_arrow_repeat_all_off_24_regular;
		}, 0, 0);
		boostBtn.setEnabled(isReblogPermitted);
